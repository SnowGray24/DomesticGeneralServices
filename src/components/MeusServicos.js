...
function MeusServicos(){
    ...
    const verificationItens = () =>{
        if (listaServico.length === 0){
            ...
            return (
                <div style={{position: 'absolute',width: '80%', height: '520px', overflow: 'scroll', scrollbarWidth: 'none'}}>
                ...
                <div style={{display: 'flex', marginTop: '2em'}}>
                    {listaServico.map(obj =>(
                        <Itens typeCard="solicitacao" chave={obj.Id} previcao={obj.Dias} titulo={obj.Titulo} image={obj.Img}  />
                    ))}
                </div>
                ...
                <div style={{display: 'flex', marginTop: '2em'}}>
                    {listaServico.map(obj =>(
                        <Itens typeCard="emAndamento" chave={obj.Id} previcao={obj.Dias} titulo={obj.Titulo} image={imgIconFinalizado}  />
                    ))}
                </div>
